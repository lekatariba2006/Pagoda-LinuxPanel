            ...
            if m_module in self.__push_model:
                ...
                if m_module in ['dingding','weixin']:
                    ...
                    sdata['sm_args'] = public.check_sms_argv({
                        ...
                        'website':clist[0]["siteName"],
                        'time':clist[0]["notAfter"],
                        'total':len(clist)
                    })
                elif m_module in ['mail']:

                    for x in clist: p_msg += "<p>网站：{}  到期：{}  </p>".format(x['siteName'],x['notAfter'])

                    sdata['title'] = '宝塔面板SSL到期提醒'
                    sdata['msg'] = """
                        <p>服务器 ：{}</p>
                        <p>检测时间 ：{}</p>
                        <p>即将到期 ：<font color=#ff0000>{} 张</font></p>
                        {}
                    """.format(public.GetLocalIp(),public.format_date(),str(len(clist)),p_msg)
